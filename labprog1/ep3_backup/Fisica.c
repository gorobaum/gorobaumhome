...
{
	Vetor v1, v2;
	double Vel[4], m[2], Qx, Qy, massaTotal;
	...
	v1->x = (Qx + m[1]*CoefRest*(Vel[2] - Vel[0])) / massaTotal;
	v1->y = (Qy + m[1]*CoefRest*(Vel[3] - Vel[1])) / massaTotal;

	v2->x = (Qx + m[0]*CoefRest*(Vel[0] - Vel[2])) / massaTotal;
	v2->y = (Qy + m[0]*CoefRest*(Vel[1] - Vel[3])) / massaTotal;
}
