...
link NEW (Vertex w, link next, char symbol) {
...
int **MATRIXinit (int r, int c, char symbol) {
    Vertex i, j;
    char **m;
...
void DIGRAPHinsertA (Digraph G, Vertex v, Vertex w, char symbol) {
    link p;
    if ( w == v ) return;
    for ( p = G->adj[v]; p != NULL; p = p->next) if ( p->w == w) return;
    G->adj[v] = NEW(w, G->adj[v], symbol);
    G->A++;
}
